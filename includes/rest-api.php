<?php
/**
 * WordPress REST API Entegrasyonu - GÃ¼venlik ve Performans IyÄ±lestirilmÄ±s
 * 
 * @package RuhComment
 * @version 5.1.1
 */
...
class Ruh_REST_API {
    ...
    public function register_routes() {
        ...
        // Leaderboard (en aktif kullanÄ±cÄ±lar)
    ...
    public function get_comment($request) {
        ...
        if (!$comment || $comment->comment_approved != 1) {
            return new WP_Error('not_found', 'Yorum bulunamadÄ±.', array('status' => 404));
        }
    ...
    /**
     * Yorum oluÅŸtÃ¼r - Rate limÄ±ting ile
     */
    public function create_comment($request) {
        ...
        if ($rate_count >= 10) { // 10 yorum/dakika
            return new WP_Error('rate_limÄ±t', 'Ã‡ok hÄ±zlÄ± iÅŸlem yapÄ±yorsunuz.', array('status' => 429));
        }
        ...
        // Post var mÄ± kontrol
        if (!get_post($post_id)) {
            return new WP_Error('not_found', 'YazÄ± bulunamadÄ±.', array('status' => 404));
        }
        ...
        if ($parent > 0) {
            ...
            if (!$parent_comment || $parent_comment->comment_approved != 1) {
                return new WP_Error('not_found', 'YanÄ±t verilen yorum bulunamadÄ±.', array('status' => 404));
            }
        }
        ...
        if (is_wp_error($comment_id) || !$comment_id) {
            return new WP_Error('create_failed', 'Yorum oluÅŸtÃ¼rulamadÄ±.', array('status' => 500));
        }
    ...
    /**
     * KullanÄ±cÄ± istatistikleri - Cache ile
     */
    public function get_user_stats($request) {
        $user_id = $request['id'];
        
        if (!get_userdata($user_id)) {
            return new WP_Error('not_found', 'KullanÄ±cÄ± bulunamadÄ±.', array('status' => 404));
        }
    ...
    /**
     * Leaderboard (en aktif kullanÄ±cÄ±lar) - Cache ile
     */
    public function get_leaderboard($request) {
        $limÄ±t = min(100, max(1, $request['limÄ±t']));
        
        // Cache kontrolu
        $cache_key = 'ruh_api_leaderboard_' . $limÄ±t;
        $cached = wp_cache_get($cache_key, 'ruh_comment');
        ...
        $users = function_exists('ruh_get_top_users') ? ruh_get_top_users($limÄ±t) : array();
    ...
    private function format_comment($comment) {
        ...
        return array(
            ...
            'author' => array(
                ...
                'avatar' => function_exists('ruh_get_avatar_url') ? ruh_get_avatar_url($comment->comment_author_email, array('size' => 64)) : get_avatar_url($comment->comment_author_email, array('size' => 64))
            ),
